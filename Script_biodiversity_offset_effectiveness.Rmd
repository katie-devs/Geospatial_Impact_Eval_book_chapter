...
In this exercise you will replicate the key analytical steps of the geospatial impact evaluation conducted by Devenish et al., 2022, described in the chapter *Offsetting forest loss from a major infrastructure development* in the Geospatial Impact Evaluation book.

The aims of this exercise are to 
1) conduct a geospatial impact evaluation of a policy intervention aimed at reducing deforestation by replicating the key analytical steps in Devenish et al., and inspecting the results. 
2) learn how to test whether results are robust to alternative matching model specifications,
3) gain confidence in applying these tools and methodologies to your own research questions.


The sections below contain pre-prepared code to execute the key analytical steps described in the chapter. Following along with the chapter, you will work sequentially through the steps below, running the code on the input data provided or your own data. You will not need to write new code, only alter the file paths, run the code and inspect the results.



# Load Libraries

If you have not already installed these packages you will need to install them. 

```{r loading libraries, message = FALSE}

# install.packages("package_name")   To install packages delete # and replace package_name with the name of the package(s) required.
...
ANK <- read.csv("Input_data_for_exercise/Ankerana_sample.csv")
Cont <- read.csv("Input_data_for_exercise/Control_sample.csv")

ANK <- ANK[, -1]   # Remove unnecessary first column.
Cont <- Cont[, -1]
...
substituting the parameters (caliper, method, distance and replace) of the
...
Inspect the summary statistics from the matching:

```{r}

View(Main_sum[[4]])   # Check whether the Std. Mean Diff for the matched data is less than 0.2

View(Main_sum[[2]])   # Check the number of treated units which were matched.

print(Main_sum)      # You can compare the covariate balance (Std. Mean Diff) before and after matching by comparing the printed Summary of Balance for All Data (pre-matching) to the Summary of Balance for Matched data  

```



## 2b) Extract key summary statistics and combine into one dataframe which we call cov_balance.
...
# 3) Aggregate the matched pixel data by year and group (offset or control)
...
## 3a) Aggregate the matched pixel data by year and group 

Aggregate the matched pixel data by year and group by counting the number of pixels deforested each year (1-19 in the Tree_loss variable) within the offset and comparison sample. This produces a long-format dataframe where there are 38 observations, one for each year for both the offset and comparison sample. The value in Annual_Deforestation gives the total count of deforestation per year in each sample. 
...
## 3c) Transform the outcome variable 
...
log_annual_defor is now used as the outcome variable from now on.
...
## 4a) Subset the data to leave only the pre-intervention years
...
## 4b) Run the regression to test for parallel trends
...
# 5) Run the DiD regression

## 5a) Run the regression 
...
## 5b) Back-transform the coefficient of interest

Back-transforming means the coefficient represents the percentage reduction in deforestation after protection in Ankerana, relative to the estimated counterfactual.
...
# 6) Run a fixed effects panel regression on data from all four offsets

## 6a) Load pre-prepared, matched datasets for the other three offsets, tidy and merge with Ankerana data

```{r}

# Load pre-prepared matched and aggregated data for the other three offsets:

CZ_FE_dat <- read.csv('Input_data_for_exercise/CZ_FE_dat.csv')
CFAM_FE_dat <- read.csv('Input_data_for_exercise/CFAM_FE_dat.csv')
TTF_FE_dat <- read.csv('Input_data_for_exercise/TTF_FE_dat.csv')

ANK_FE_dat <- annual_defor


# Change the column names in the data the other offsets to match those of Ankerana. Create function and then apply to all new datasets.

tidy_data <- function(data){
  data <- data[, -1] %>%         # Remove the first column X
    rename(Time = TimeF) %>%     # Rename columns to match those in ANK_FE_dat
    rename(Treated = TreatedF)
}

CZ_FE_dat <- tidy_data(CZ_FE_dat)
CFAM_FE_dat <- tidy_data(CFAM_FE_dat)
TTF_FE_dat <- tidy_data(TTF_FE_dat)


# Change the name of the matched control samples in each of the datasets. They are currently all called 'Cont' so when the data are pooled, the plm function will assume these observations are all from a single group, leading to an unbalanced panel. We need to change the names to ensure that when the data are pooled, each control sample is treated as a separate sample.

CZ_FE_dat$Sample[CZ_FE_dat$Sample == 'Cont'] <- 'CZ1'
CFAM_FE_dat$Sample[CFAM_FE_dat$Sample == 'Cont'] <- 'CFAM1'
TTF_FE_dat$Sample[TTF_FE_dat$Sample == 'Cont'] <- 'TTF1'

# Make the sample variable a factor

CZ_FE_dat$Sample <- as.factor(CZ_FE_dat$Sample)
CFAM_FE_dat$Sample <- as.factor(CFAM_FE_dat$Sample)
TTF_FE_dat$Sample <- as.factor(TTF_FE_dat$Sample)

# Add a sample column to ANK_FE_dat

ANK_FE_dat$Sample <- ifelse(ANK_FE_dat$Treated == 1, 'ANK', 'ANK1')

# Factorise

ANK_FE_dat$Sample <- as.factor(ANK_FE_dat$Sample)

# Re-order columns in ANK_FE_dat to match the other datasets

ANK_FE_dat <- ANK_FE_dat[, c(1,6, 3, 4, 2, 5)]


# Merge all four offset-control datasets

FE_dat <- rbind(ANK_FE_dat, CFAM_FE_dat, CZ_FE_dat, TTF_FE_dat)

# We need to factorise the year variable

FE_dat$Year <- factor(FE_dat$Year)


# Make one variable indicating treated status. Tr = 1 for observations from an offset after protection and 0 for observations from an offset before protection and the control sample. Cannot use Treated and Time and the interaction because these are collinear with the fixed effects. 

FE_dat$Tr <- ifelse(FE_dat$Treated== "1" & FE_dat$Time== "1",1,0) 


```


## 6b) Run the fixed effects panel regression

Tr is the coefficient of interest in this regression. A significant negative coefficient indicates there was significant reduction in the log-transformed count of deforestation following protection of the four biodiversity offsets.
...
## 6c) Back-transform the estimates.

```{r}

(exp(coef(FE_all))-1)*100   
(exp(confint(FE_all))-1)*100   

```
Interrogate these results. Did offset protection reduce deforestation on average across all four offsets?

...

Yes! The coefficient of Tr is negative and significant. Backtransforming this estimate shows that protection reduced deforestation across the entire offset portfolio by an average of 58% per year, relative to the estimated counterfactual. The 95% confidence interval on this estimate is 37 - 73%.




# 7)	Check the robustness of these results to alternative matching specifications

Check the result for Ankerana are robust to alternative matching model choices. We
...
## 7a) Run the_parameters_loop to test the effect of varying the statistical distance measure and model parameters
...
names(loop_parameters)

```


The output dataframe is called loop_parameters. Each row contains the results from one alternative modelling specification. The specification is indicated in columns 7- 21 with a boolean TRUE/FALSE value indicating whether that parameter was included in that specification or not. 

Column 1 contains the raw estimated treatment effect of protection on deforestation within the Ankerana offset. You can back-transform to obtain percentage reduction in deforestation using method shown in Step 5b. 
Column 2 is the standard error of the estimate.
Column 3 shows the number of treated units which could not be matched.
Columns 4 and 5 contain the mean and max standardised mean difference between the treated and matched control sample, averaged across all variables. 
Column 6 indicates whether the matched data show parallel trends in the pre-treatment period.


## 7b) Run the_cov_loop to test the effect of including additional covariates
...
the_cov_loop(data, 11, 1)
#write.csv(synthesis_loop_cov,"Output/loopCFAM_cov.csv")

names(synthesis_loop_cov)

```

The output dataframe is synthesis_loop is contains the same structure as loop_parameters.


## 7c) Merge outputs from the two loops:
...
## 7e) Explore the remaining results. 

Try plotting the coefficients, calculating the standard deviation, or extracting the highest 10% and
